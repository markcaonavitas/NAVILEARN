...
namespace ParseManager.Droid
{
    public class ParseClientManager : IParseClientManager
    {
        ...
        public async Task Transmit(string DeviceName)
        {
            try
            {
                var currentSession = await ParseSession.GetCurrentSessionAsync();
                ...
                datalogId = Regex.Match(parseFile.Url.ToString(), @"(?<=\/)(\w)*(?=[_])", RegexOptions.IgnoreCase).Value;
        ...
        public async Task<string> Login(string Username, string Password)
        {
            try
            {
                ...
                if (hasLoginBeenRequired)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine(e.Message + "Parse Login: maybe no Internet to login");
        ...
        public async Task<bool> IsEmailVerified(string email)
        {
            ...
            {
                return (bool)userObject["emailVerified"];
            }
            else
        ...
        public async Task RequestPasswordReset(string emailAddress)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("RequestPasswordReset Exception" + e.Message);
                throw;
            }
        }
